...
class weisrc_dishModuleSite extends WeModuleSite
{
    ...
    public function doMobileWapList()
    {
      ...
      //  die();
        ...
        if(!empty($logid)){
          ...
          $opp = $_GPC['opp'];
            $url = "index.php?i=1&c=entry&do=opera&m=weisrc_dish&tablesid=".$tablesid."&opp=".$opp;
            header("location:$url");
        ...
        $children = array();
        //不显示分类
        $category = pdo_fetchall("SELECT * FROM " . tablename($this->table_category) . " WHERE weid = :weid AND storeid=:storeid  and name != '特殊类' ORDER BY  displayorder DESC,id DESC", array(':weid' => $weid, ':storeid' => $storeid));
    ...
    //     var_dump($from_user);
     ...
     //   $from_user = 'testuser';
        ...
        $cart = pdo_fetchall("SELECT *,a.taste as flavor FROM " . tablename($this->table_cart) . " a LEFT JOIN " . tablename('weisrc_dish_goods') . " b ON a.goodsid=b.id WHERE a.weid=:weid AND (a.from_user=:from_user or a.tablesid=:tables) AND a.storeid=:storeid group by a.goodsid order by a.id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':storeid' => $storeid, ':tables' => $tables));
        $totalcount = 0;
        $totalprice = 0;
        foreach ($cart as $key => $value) {
            if($value['tasteid'] != ''){
...
//var_dump($from_user);
 ...
 status=1 ORDER BY id DESC ", array(':weid' => $this->_weid, ':from_user' => $from_user));
        ...
        //已确认
        $order_list = pdo_fetchall("SELECT a.* FROM " . tablename($this->table_order) . " AS a LEFT JOIN " . tablename($this->table_stores) . " AS b ON a.storeid=b.id  WHERE  a.from_user='{$from_user}' ORDER BY a.id DESC LIMIT 20");
       // var_dump($order_list);
        //die();
        //数量
...
//        $from_user = 'testuser';
        ...
        if (checksubmit()) {
            ...
            $url2 = "index.php?i=1&c=entry&do=orderdetail&m=weisrc_dish&orderid=".$sqlorder['orderid'];
    ...
    public function doMobileEvaluate()
    {
        ...
        $idd = $_GPC['id'];
        $orderid = $_GPC['orderid'];
        $url = "index.php?i=1&c=entry&do=orderdetail&m=weisrc_dish&orderid=".$orderid;
...
// if ($logid =='') {
//     $urll = 'http://shanghai.xyj0772.com/app/index.php?i=1&c=entry&do=index&m=j_money';
//    message('未登录', $urll, 'success');
// }
...
public function doMobileGuqing()
{
        ...
        $url = 'index.php?i=1&c=entry&do=caipin&m=weisrc_dish';
...
// $fish = pdo_query("SELECT id FROM  " . tablename($this->table_goods) . "  WHERE pcate =1");
// var_dump($fish);
...
if($opera['tablesid']==""){
  ...
  //修改鱼与其他菜品步骤相同10.24.2016
    ...
    if($opp==""){
        ...
        //称鱼处理
            $list = pdo_fetch("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=1,step1=".$time.",step1man='".$ygname."' WHERE weid = '{$weid}' and goodsid in(SELECT id FROM  " . tablename($this->table_goods) . "  WHERE pcate =1) and status=1 and stepnow=0 and tablesid=".$opera['tablesid']);
                $list_url ='index.php?i=1&c=entry&do=chenyu&m=weisrc_dish';
                header("location:$list_url");
    }elseif($opp=="sha"){ 
        //杀鱼处理
            $list = pdo_fetch("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=2,step2=".$time.",step2man='".$ygname."' WHERE weid = '{$weid}' and goodsid in(SELECT id FROM  " . tablename($this->table_goods) . "  WHERE pcate =1) and status=1 and stepnow=1 and tablesid=".$opera['tablesid']);
            // var_dump("成功");
                $list_url ='index.php?i=1&c=entry&do=shayu&m=weisrc_dish';
            message('扫描成功,感谢您的辛勤劳动', $list_url, 'success');
                // header("location:$list_url");
    }elseif($opp=="yan"){ 
        //腌鱼处理
            $list = pdo_fetch("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=3,step3=".$time.",step3man='".$ygname."' WHERE weid = '{$weid}' and goodsid in(SELECT id FROM  " . tablename($this->table_goods) . "  WHERE pcate =1) and status=1 and stepnow=2 and tablesid=".$opera['tablesid']);
                $list_url ='index.php?i=1&c=entry&do=yanyu&m=weisrc_dish';
            message('扫描成功,感谢您的辛勤劳动', $list_url, 'success');
                // header("location:$list_url");
    }  
...
$list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_stores) . " as c on a.storeid=c.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  WHERE a.weid = '{$weid}' and a.dateline >=$start and a.dateline <$end and a.status=1 order by a.reminder desc,a.dateline");  //and a.stepnow= {$opera['step']}  and a.stepnow >0
...
$list = pdo_fetchall("SELECT * from ims_weisrc_dish_tables where status>=1 order by tablezonesid DESC,title ASC");
foreach($list as $key => $value){
    $list[$key]['name']=pdo_fetch("SELECT title from ims_weisrc_dish_tablezones where id=".$value['tablezonesid']);
}
// var_dump($list);
...
$list = pdo_fetchall("SELECT a.title,b.price,b.total from ims_weisrc_dish_goods as a join ims_weisrc_dish_order_goods as b on a.id=b.goodsid where b.status=1 and b.tablesid=".$tables);
foreach($list as $key=>$value){
...
$totalprice = pdo_fetch("SELECT SUM(price*total) as totalprice from ims_weisrc_dish_order_goods where status=1 and tablesid=".$tables);
    ...
    //         $back_url ='index.php?i=1&c=entry&do=fish&m=weisrc_dish';
    //         header("location:$back_url");
    //     }else{

    //         $back_url ='index.php?i=1&c=entry&do=opera&m=weisrc_dish';
    //         header("location:$back_url");
    //     }
    //     $mobile = $_COOKIE['mobile'];
    ...
    public function doMobilelogin(){
        ...
        if($logid!=''){
            ...
            if($item['pcate']==3){
                $log_url ='index.php?i=1&c=entry&do=operalist&m=weisrc_dish';
            ...
            //$out_url ='http://shanghai.xyj0772.com/app/index.php?i=1&c=entry&do=index&m=j_money';
            message('登录失败！');
        }
...
//退出登录
    public function doMobilelogout()
    {
        ...
        setcookie("pcate",'');
        setcookie("logid",'');
        $out_url ='index.php?i=1&c=entry&do=index&m=j_money';
...
//        die('<script>location.href = "' . $url . '";</script>');
    ...
    public function doMobileAddToOrder()
    {
        ...
        //查询购物车
        $cart = pdo_fetchall("SELECT * FROM " . tablename($this->table_cart) . " WHERE weid = :weid AND (from_user = :from_user or tablesid= :tables) AND storeid=:storeid order by id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':storeid' => $storeid, ':tables' => $tables), 'goodsid');
        // var_dump($cart);
        // die();
        if ($rtype != 1) {
            if (empty($cart)) { //购物车为空
...
//            message($status);
            ...
            pdo_update($this->table_tables, array('status' => 0), array('id' => $tablesid, 'weid' => $weid));
            pdo_update($this->table_tables, array('status' => $status), array('id' => $tablesnet, 'weid' => $weid));
            pdo_update($this->table_order, array('tables' => $tablesnet), array('tables' => $tablesid, 'weid' => $weid));
            $sql=pdo_fetch("SELECT id from " . tablename($this->table_order) . " where tables=".$tablesnet." order by id desc");
            pdo_update($this->table_order_goods, array('tablesid' => $tablesnet), array('orderid' => $sql['id'], 'weid' => $weid));
            message('操作成功！', $this->createWebUrl('tables', array('op' => 'display', 'storeid' => $storeid)), 'success');
        }
            ...
            if (!empty($list)) {
                foreach ($list as $row) {
                    $userids[$row['from_user']] = $row['from_user'];
                        ...
                        //昨天销售额
            $yesterday = pdo_fetch("SELECT SUM(totalprice) as yest FROM ims_weisrc_dish_order WHERE ispay=1 AND dateline>$yeststart and dateline<=$yestend");
            //本月销售额
            $month = pdo_fetch("SELECT SUM(totalprice) as month FROM ims_weisrc_dish_order WHERE ispay=1 AND dateline>$month_start and dateline<=$month_end");
            //累计销售额
            $moneyall = pdo_fetch("SELECT SUM(totalprice) as moneyall FROM ims_weisrc_dish_order WHERE ispay=1");

            //销售排行
            $goodsRank = pdo_fetchall("SELECT * FROM " . tablename($this->table_goods) . "");
